...
def owner_item_get_all(id):
    ...
    try:
        ...
        sql = "SELECT * FROM item WHERE item_status != 'off' AND item_owner = ? ORDER BY item_date DESC"
        cursor.execute(sql, (id,))
        items_rows = cursor.fetchall()
        conn.close()
...
def item_all_get_all(id):
    ...
    # Obtém todos os registros válidos de 'item' identificado pelo 'id', juntamente com os dados de 'owner' correspondente.
    # Request method → GET
